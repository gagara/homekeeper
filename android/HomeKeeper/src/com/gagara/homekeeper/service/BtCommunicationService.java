...
public class BtCommunicationService extends Service {
    ...
    private void initOngoingNotification() {
        ...
        if (dev != null) {
            serviceNotification.setContentText(dev.getName() + ": " + getText(state.toStringResource()));
        } else {
                ...
                while (true) {
                    try {
                        ...
                        try {
                            ...
                            Log.e(TAG, "failed to parse [" + msg + "]: " + e.getMessage(), e);
                        }
                    } catch (IOException e) {
                        Log.e(TAG, e.getMessage(), e);
                        state = State.ERROR;
        ...
        if (msgType == ControllerConfig.MessageType.CLOCK_SYNC) {
            ...
            if (clocksDelta != null) {
                ...
                if (msgType == ControllerConfig.MessageType.CURRENT_STATUS_REPORT) {
                    CurrentStatusResponse stats = new CurrentStatusResponse(clocksDelta).fromJson(json);
                    if (stats != null) {
                    ...
                    NodeStateChangeResponse nodeState = new NodeStateChangeResponse(clocksDelta).fromJson(json);
                    if (nodeState != null) {
    ...
    private class ControllerCommandsReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Constants.CONTROLLER_CONTROL_COMMAND_ACTION.equals(action)) {
                NodeStateChangeRequest command = intent.getParcelableExtra(Constants.COMMAND_KEY);
                long currTimestamp = new Date().getTime();
                if (state == State.ACTIVE && out != null && clocksDelta != null
                        && currTimestamp < command.getValidityTimestamp().getTime()) {
                    try {
                        out.write(command.toJson().toString().getBytes());
                    } catch (IOException e) {
                        Log.e(TAG, "failed to send command: " + command.toString() + ": " + e.getMessage(), e);
                    }
                } else {
